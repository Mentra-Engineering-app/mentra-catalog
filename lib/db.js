...
// Mentra Sync Log
export async function createSyncLog(lessonId) {
  const res = await query(
    'INSERT INTO mentra_sync_log (lesson_id) VALUES ($1) RETURNING *',
    [lessonId]
  )
  return res.rows[0]
}

export async function updateSyncLog(id, { sync_status, mentra_microtraining_id, error_message }) {
  const fields = []
  const values = []
  let paramIndex = 1
  ...
  if (mentra_microtraining_id !== undefined) {
    fields.push(`mentra_microtraining_id = $${paramIndex++}`)
    values.push(mentra_microtraining_id)
  }
  ...
  values.push(id)
  const res = await query(
    `UPDATE mentra_sync_log SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
    values
  )
  return res.rows[0]
}

export async function getSyncLogForLesson(lessonId) {
  const res = await query(
    'SELECT * FROM mentra_sync_log WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT 1',
    [lessonId]
  )
  return res.rows[0] || null
}

export async function getUnsyncedApprovedLessons() {
  const res = await query(`
    ...
    WHERE l.approved = true
      AND NOT EXISTS (
        SELECT 1 FROM mentra_sync_log sl
        WHERE sl.lesson_id = l.id AND sl.sync_status = 'synced'
      )
...
// Companies with mentra_org_id
export async function updateCompanyMentraOrgId(companyId, mentraOrgId) {
  const res = await query(
    'UPDATE companies SET mentra_org_id = $1 WHERE id = $2 RETURNING *',
    [mentraOrgId, companyId]
  )
  return res.rows[0]
}

export async function getCompanyByMentraOrgId(mentraOrgId) {
  const res = await query(
    'SELECT * FROM companies WHERE mentra_org_id = $1',
    [mentraOrgId]
  )
  return res.rows[0] || null
}
